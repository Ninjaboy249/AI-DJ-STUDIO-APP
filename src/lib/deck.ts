...
export function buildDeckSignal(s: DeckState): DeckSignal | null {
  ...
  const rawPosition = el.add(base, el.accum(inc, seekTrig));

  // Loop wrap: loopIn + flooredMod(rawPosition - loopIn, loopOut - loopIn)
  // Uses floored-modulo (x - len*floor(x/len)) because el.mod is fmod and
  // keeps the dividend's sign, which breaks the wrap for values < loopIn.
  let position: NodeRepr_t;
  if (s.looping && s.loopOut > s.loopIn) {
    const loopInN = el.const({ key: `${s.id}_loopIn`, value: s.loopIn });
    const loopLen = el.const({ key: `${s.id}_loopLen`, value: s.loopOut - s.loopIn });
    const shifted = el.sub(rawPosition, loopInN);
    // floored mod: shifted - loopLen * floor(shifted / loopLen)
    const floored = el.mul(loopLen, el.floor(el.div(shifted, loopLen)));
    position = el.add(loopInN, el.sub(shifted, floored));
  } else {
